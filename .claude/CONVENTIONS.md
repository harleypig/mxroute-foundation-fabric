...
## Toolchain — real Terraform + a dev-override (not Docker)

Unlike harleydev (which runs Terraform through Docker), this repo uses a
**locally installed `terraform`**, because the dev-override needs Terraform to
load the provider **binary** directly from the filesystem — a Docker mount
would complicate the absolute path the override points at.

- **Provider availability (local dev):** the `harleypig/mxroute` provider is
  **not yet published** to the Registry. Build it from the sibling
  `../terraform-provider-mxroute` checkout into `.dev/` and point Terraform at
  it via [`dev.tfrc`](../dev.tfrc) (`TF_CLI_CONFIG_FILE`). A dev-override makes
  Terraform skip `init` for that provider and warn — expected. **Once the
  provider is published, drop `dev.tfrc` and consume it from the Registry.**
- **Provider availability (CI):** CI downloads the provider's published
  **v0.1.0 GitHub release** binary into a filesystem mirror and points
  Terraform at it the same way — no Go build in this repo's CI, and no wait for
  the Registry.
- **terraform-docs** generates each module's README tables (config:
  `.terraform-docs.yml`, injected between the `BEGIN_TF_DOCS`/`END_TF_DOCS`
  markers). A module keeps hand-written prose around the generated block.
...
| 2 | Lint | **Planned** | `tflint` per module — tracked in TODO. |
| 3 | Type-check | **N/A** | HCL. |
| 4 | Code smell | **Off** | `arch-review` ad hoc. |
| 5 | Security | **Planned** | `trivy config` over modules — tracked in TODO. Secrets: `gitleaks` + `detect-private-key` Active. |
...
| 12 | Build | **Active** | `terraform validate` per module (CI, via the dev-override mirror). |
| 13 | Documentation | **Active** | terraform-docs-generated READMEs, gated by a docs-check. |
